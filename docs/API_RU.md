...
## 9. Примеры кода — потребление `state`

Короткие паттерны, показывающие как бот читает `state` (dict, описанный в § 3). Все сниппеты предполагают, что `decide(state)` возвращает одно из `"W" | "A" | "S" | "D" | "N"` и вызывается ~10 раз в секунду. Держите её быстрой: без файлового I/O, без сети, без медленных импортов.
...
Следующий шаг приземляется в `(me.x + dx, me.y + dy)`. Если эта клетка вне `[0, map_w) × [0, map_h)` — вы умираете с `reason="out_of_bounds"`. `state["map_w"]` и `state["map_h"]` приходят из фрейма `welcome`, который ваша обвязка сохранила до первого `state`.
...
`state["view"].trail` — 2D-массив, индексируется `[ry][rx]` по view-окну. Каждая клетка — это pid владельца trail (или `0`). Шаг на любую ненулевую и не-свою клетку убивает этого игрока.
...
`state["scores"]` приходит примерно раз в 5 секунд. Между этим поле отсутствует (или содержит тот же закэшированный объект). У каждой записи есть `pid`, `name`, `area`, `kills`, `deaths`.
...
### Применение `captured`-delta

Событие `captured` (см. § 3) перечисляет каждую клетку, только что перешедшую в вашу зону. Применяйте delta к fog-of-war памяти немедленно — без этого клетки вне live view не перекрасятся, пока вы не пройдёте через них снова.
...
- **Manhattan** (`abs(dx) + abs(dy)`) — дёшево, игнорирует препятствия. Подходит для «этот игрок рядом?» / «кто из этих ближе?».
- **Чебышёв** (`max(abs(dx), abs(dy))`) — соответствует квадратному view-окну, центрированному на игроке.
- **BFS по grid** — только когда важна *достижимая* дистанция (например, обход чужой зоны). Держите BFS внутри view, если только реально не нужна дальняя достижимость — full-map BFS по 786 K клеток на 10 Hz — не дёшево.
...
«Когда возвращаться домой» и прочие стратегические эвристики — см. [RULES.md](RULES_RU.md) § 13 (анти-паттерны и эвристики).
